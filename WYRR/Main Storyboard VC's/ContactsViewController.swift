...
class ContactsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    override func viewDidLoad() {
        ...
        table_view.sectionIndexColor = UIColor.init(white: 1, alpha: 0.4)
        registerContactCell()
    ...
    fileprivate func registerContactCell() {
        ...
        if let bundleURL = podBundle.url(forResource: EPGlobalConstants.Strings.bundleIdentifier, withExtension: "bundle") {
            ...
            if let bundle = Bundle(url: bundleURL) {
                ...
                table_view.register(cellNib, forCellReuseIdentifier: "Cell")
            }
            else {
...
//    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let returnedView = UIView(frame: CGRect(x: 0, y: 0, width: self.view.frame.size.width, height: 22)) //set these values as necessary
        returnedView.backgroundColor = UIColor(white: 1, alpha: 0.1)

        let label = UILabel(frame: CGRect(x: 16, y: 0, width: 100, height: 25))
